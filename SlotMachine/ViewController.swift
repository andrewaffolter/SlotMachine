...
//  SlotMachine
//
//  Created by Andrew Affolter on 3/14/15.
...
class ViewController: UIViewController {
    ...
    //using a CG float because the view parameters accept type CGFloat. CGFloats automatically type def to Doubles on 64-bit systems
    let kMarginForView:CGFloat = 10.0
    //using the lower-case k to indicate it's a constant
    let kSixth:CGFloat = 1.0/6.0
    
    //number of columns
    let kNumberOfContainers = 3
    //number of slots (rows)
    ...
    func betOneButtonPressed(button:UIButton){
        ...
        }else
        {
            if currentBet < 5{
                currentBet += 1
                credits -= 1
                //Call update main view to update the labels
                updateMainView()
            }else
            {
    ...
    func spinButtonPressed(button: UIButton){
        //remove all the existing image views before adding new ones.
        removeSlotImageViews()
        //use the Factory to create slots
        slots = Factory.createSlots()
        //whenever we create we should reset the secondContainer to setup the new ImageViews
    ...
    func setupContainerViews() {
        ...
        self.self.secondContainer = UIView(
            frame: CGRect(
                x: self.view.bounds.origin.x + kMarginForView,
        ...
        self.self.thirdContainer = UIView(
            frame: CGRect(
                x: self.view.bounds.origin.x + kMarginForView,
        ...
        self.self.fourthContainer = UIView(
            frame: CGRect(
                x: self.view.bounds.origin.x + kMarginForView,
    ...
    func setupFirstContainer(containerView:UIView){
        ...
        //auto strech the width and height to fit the font
    ...
    //this function has a default parameter header
    func showAlertWithText(header: String = "Warning", message: String){
        //Create an alert to display on the UI.
        var alert = UIAlertController(title: header, message: message, preferredStyle: UIAlertControllerStyle.Alert)
        //Give the user the ability to dismiss this alert by adding an Ok button
